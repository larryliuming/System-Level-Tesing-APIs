...
feature -- Element change
	...
	separate_argument (a_string: STRING): LIST [STRING]
			-- Separate arguemtns in `a_string'
		require
			not_void: a_string /= Void and then not a_string.is_empty
		local
			l_string: STRING
			l_pos, l_last_pos: INTEGER
		do
			...
			from
				...
				l_pos := first_white_position (l_string)
			until
				l_pos = 0
			loop
				Result.extend (l_string.substring (l_last_pos, l_pos - 1))
				l_last_pos := l_pos
				l_string := l_string.substring (l_pos + 1, l_string.count)
				l_pos := first_white_position (l_string)
			end
	...
	first_white_position (a_s: STRING): INTEGER
			-- Position of first white space character in
			-- `a_s' or 0 if no white space characters.
			-- This feature is useful for separating arguments
		require
			string_not_void: a_s /= Void
		local
			l_pos: INTEGER
			l_found: BOOLEAN
			l_char: CHARACTER
		do
			from
				l_pos := 1
			until
				l_pos > a_s.count or l_found
			loop
				l_char := a_s.item (l_pos)
				if is_white (l_char) then
					l_found := True
				else
					l_pos := l_pos + 1
				end
